 ...
 *   TELEGRAM_OWNER_CHAT_ID — target chat ID
 */
...
export function buildDigestMessage(opts: DigestOptions): string {
  const { topics, runId, totalPosts, uniquePosts, subredditCount, durationMs } = opts;
  ...
  const lines: string[] = [
    `🧠 *Intelligence Digest — ${escapeMd(now)}*`,
    "",
  ];
  ...
  for (const topic of topics.slice(0, 10)) {
    ...
    if ((topic.delta === "up" || topic.delta === "down") && topic.scoreDelta != null) {
      const pct = topic.scoreDelta > 0 ? `+${topic.scoreDelta}%` : `${topic.scoreDelta}%`;
...
export async function sendTelegramDigest(opts: DigestOptions): Promise<boolean> {
  if (!BOT_TOKEN || !CHAT_ID) {
    console.warn("[telegram] Missing TELEGRAM_BOT_TOKEN or TELEGRAM_OWNER_CHAT_ID — skipping");
    return false;
  }

  const hasAlerts = opts.topics.some((t) => t.delta === "new" || t.delta === "up");

  if (!hasAlerts && !opts.force) {
    console.log("[telegram] No new/up topics — skipping notification (use --force to override)");
    return false;
  }
